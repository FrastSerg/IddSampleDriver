...
static ::EVT_IDD_CX_MONITOR_QUERY_TARGET_MODES            MonitorQueryModes;
...
// This macro creates the methods for accessing an DCWrapper as a context for a WDF object
WDF_DECLARE_CONTEXT_TYPE(DCWrapper);
...
_Use_decl_annotations_
static ::NTSTATUS DeviceAdd( ::WDFDRIVER Driver, ::PWDFDEVICE_INIT pDeviceInit) {
  ...
  WDF_OBJECT_ATTRIBUTES_INIT_CONTEXT_TYPE( &Attr, DCWrapper );
  Attr.EvtCleanupCallback = []( ::WDFOBJECT Object) {
    // Automatically cleanup the context when the WDF object is about to be deleted
    ::DCWrapper* pContext{ ::WdfObjectGet_DCWrapper(Object) };
    if( pContext ) {
      pContext->Cleanup();
    }
  };
  ...
  DCWrapper* pContext = ::WdfObjectGet_DCWrapper( Device );
...
_Use_decl_annotations_
static ::NTSTATUS DeviceD0Entry( ::WDFDEVICE Device, ::WDF_POWER_DEVICE_STATE PreviousState ) {
...
{
  m_hTerminateEvent.Attach( ::CreateEventW(nullptr, FALSE, FALSE, nullptr) );
...
void SwapChainProcessor::RunCore() {
  ...
  ::IDARG_IN_SWAPCHAINSETDEVICE SetDevice{ DxgiDevice.Get() };
...
constexpr ::UINT32 operator ""_KHz( unsigned long long val ) {
  return ::UINT32(val) * 1000;
}

constexpr ::UINT32 operator ""_MHz( unsigned long long val ) {
  return ::UINT32(val) * 1000000;
}

constexpr ::DISPLAYCONFIG_VIDEO_SIGNAL_INFO dispinfo( UINT32 h, UINT32 v ) {
  const ::UINT32 clock_rate = 60 * (v + 4) * (v + 4) + 1_KHz;
  return {
...
static const ::DISPLAYCONFIG_VIDEO_SIGNAL_INFO knownMonitorModes[]  {
  // 640 x 480 @ 60Hz
  {
      25249_KHz,                                     // pixel clock rate [Hz]
    { 25249_KHz, 640 + 160 },                        // fractional horizontal refresh rate [Hz]
    { 25249_KHz, (640 + 160) * (480 + 46) },         // fractional vertical refresh rate [Hz]
...
#ifndef SUTK_VAL2STR
  #define _SUTK_VAL2STR(x) #x
  #define SUTK_VAL2STR(x) _SUTK_VAL2STR(x)
#endif
...
void IndirectDeviceContext::InitAdapter() {
  ...
  AdapterCaps.MaxMonitorsSupported                 = MSP_NUM_VIRTUAL_DISPLAYS;
  AdapterCaps.EndPointDiagnostics.Size             = sizeof(AdapterCaps.EndPointDiagnostics);
  AdapterCaps.EndPointDiagnostics.GammaSupport     = IDDCX_FEATURE_IMPLEMENTATION_NONE;
  AdapterCaps.EndPointDiagnostics.TransmissionType = IDDCX_TRANSMISSION_TYPE_WIRED_OTHER;

  // Declare your device strings for telemetry (required)
  AdapterCaps.EndPointDiagnostics.pEndPointFriendlyName =     L"MSP360 virtual monitors";
  AdapterCaps.EndPointDiagnostics.pEndPointManufacturerName = L"MSP360";
  AdapterCaps.EndPointDiagnostics.pEndPointModelName =        L"MSP360 virtual monitor " SUTK_VAL2STR(MSP_VIRTUAL_DISPLAY_VERSION_MAJOR) "." SUTK_VAL2STR(MSP_VIRTUAL_DISPLAY_VERSION_MINOR);

  // Declare your hardware and firmware versions (required)
  ::IDDCX_ENDPOINT_VERSION Version { 0, };
  Version.Size                                     = sizeof(Version);
  Version.MajorVer                                 = MSP_VIRTUAL_DISPLAY_VERSION_MAJOR;
  Version.MinorVer                                 = MSP_VIRTUAL_DISPLAY_VERSION_MINOR;
  Version.Build                                    = MSP_VIRTUAL_DISPLAY_VERSION_BUILD;
  ...
  auto* pContext = ::WdfObjectGet_DCWrapper( AdapterInitOut.AdapterObject );
  pContext->pContext = this;
}

void IndirectDeviceContext::FinishInit() {
  for( unsigned int i = 0; i < MSP_NUM_VIRTUAL_DISPLAYS; i++ ) {
    CreateMonitor(i);
  }
}
...
void IndirectDeviceContext::CreateMonitor(unsigned int index) {
  ...
  ::DCWrapper* pContext = ::WdfObjectGet_DCWrapper(MonitorCreateOut.MonitorObject);
  pContext->pContext = this;
...
_Use_decl_annotations_
static ::NTSTATUS AdapterInitFinished( ::IDDCX_ADAPTER AdapterObject, const ::IDARG_IN_ADAPTER_INIT_FINISHED* pInArgs ) {
  // This is called when the OS has finished setting up the adapter for use by the IddCx driver. It's now possible
  // to report attached monitors.

  DCWrapper* pContext = ::WdfObjectGet_DCWrapper(AdapterObject);
  if( !NT_SUCCESS(pInArgs->AdapterInitStatus) ) {
    return STATUS_SUCCESS;
    //return pInArgs->AdapterInitStatus; // SU> ???
  }
...
_Use_decl_annotations_
static ::NTSTATUS AdapterCommitModes( ::IDDCX_ADAPTER AdapterObject, const ::IDARG_IN_COMMITMODES* pInArgs ) {
...
_Use_decl_annotations_
static ::NTSTATUS ParseMonitorDescription( const ::IDARG_IN_PARSEMONITORDESCRIPTION* pInArgs, ::IDARG_OUT_PARSEMONITORDESCRIPTION* pOutArgs ) {
  // ==============================
  // TODO: In a real driver, this function would be called to generate monitor modes for an EDID by parsing it. In
  // this sample driver, we hard-code the EDID, so this function can generate known modes.
  // ==============================

  pOutArgs->MonitorModeBufferOutputCount = ARRAYSIZE(knownMonitorModes);

  if( pInArgs->MonitorModeBufferInputCount < ARRAYSIZE(knownMonitorModes) ) {
    // Return success if there was no buffer, since the caller was only asking for a count of modes
    return (pInArgs->MonitorModeBufferInputCount > 0) ? STATUS_BUFFER_TOO_SMALL : STATUS_SUCCESS;
  }

  // Copy the known modes to the output buffer
  for( ::DWORD ModeIndex = 0; ModeIndex < ARRAYSIZE(knownMonitorModes); ModeIndex++ ) {
    ...
    pInArgs->pMonitorModes[ModeIndex].MonitorVideoSignalInfo = knownMonitorModes[ModeIndex];
  }
...
_Use_decl_annotations_
static ::NTSTATUS MonitorGetDefaultModes( ::IDDCX_MONITOR MonitorObject,
...
// TVSI -- Target Video Signal Info
#define TVSI mode.TargetVideoSignalInfo.targetVideoSignalInfo
...
constexpr ::IDDCX_TARGET_MODE CreateTargetMode( ::UINT Width, ::UINT Height, ::UINT VSync ) {
  ...
  TVSI.totalSize.cy                          = TVSI.activeSize.cy = Height;
  TVSI.AdditionalSignalInfo.vSyncFreqDivider = 1;
  TVSI.AdditionalSignalInfo.videoStandard    = 255;
  TVSI.vSyncFreq.Numerator                   = VSync;
  TVSI.vSyncFreq.Denominator                 = TVSI.hSyncFreq.Denominator = 1;
  TVSI.hSyncFreq.Numerator                   = VSync * Height;
  TVSI.scanLineOrdering                      = DISPLAYCONFIG_SCANLINE_ORDERING_PROGRESSIVE;
  TVSI.pixelRate                             = UINT64(VSync) * Width * Height;
  return mode;
}
#undef TVSI

_Use_decl_annotations_
static ::NTSTATUS MonitorQueryModes( ::IDDCX_MONITOR MonitorObject, const ::IDARG_IN_QUERYTARGETMODES* pInArgs, ::IDARG_OUT_QUERYTARGETMODES* pOutArgs ) {
  UNREFERENCED_PARAMETER(MonitorObject);
  ...
  pOutArgs->TargetModeBufferOutputCount = ARRAYSIZE(knownMonitorModes);
  if( pInArgs->TargetModeBufferInputCount < pOutArgs->TargetModeBufferOutputCount ) {
    return STATUS_SUCCESS;
  }

  ::IDDCX_TARGET_MODE* pMode{pInArgs->pTargetModes};
  for( const ::DISPLAYCONFIG_VIDEO_SIGNAL_INFO& md : knownMonitorModes ) {
    *pMode = CreateTargetMode( md.activeSize.cx, md.activeSize.cy, 60 );
    pMode++;
  }
  ...
  TargetModes[0]  = CreateTargetMode( 7680, 4320, 60 );
  TargetModes[1]  = CreateTargetMode( 6016, 3384, 60 );
  TargetModes[2]  = CreateTargetMode( 5120, 2880, 60 );
  TargetModes[3]  = CreateTargetMode( 4096, 2560, 60 );
  TargetModes[4]  = CreateTargetMode( 4096, 2304, 60 );
  TargetModes[5]  = CreateTargetMode( 3840, 2400, 60 );
  TargetModes[6]  = CreateTargetMode( 3840, 2160, 60 );
  TargetModes[7]  = CreateTargetMode( 3200, 2400, 60 );
  TargetModes[8]  = CreateTargetMode( 3200, 1800, 60 );
  TargetModes[9]  = CreateTargetMode( 3008, 1692, 60 );
  TargetModes[10] = CreateTargetMode( 2880, 1800, 60 );
  TargetModes[11] = CreateTargetMode( 2880, 1620, 60 );
  TargetModes[12] = CreateTargetMode( 2560, 1600, 60 );
  TargetModes[13] = CreateTargetMode( 2560, 1440, 60 );
  TargetModes[14] = CreateTargetMode( 1920, 1440, 60 );
  TargetModes[15] = CreateTargetMode( 1920, 1200, 60 );
  TargetModes[16] = CreateTargetMode( 1920, 1080, 60 );
  TargetModes[17] = CreateTargetMode( 1600, 1024, 60 );
  TargetModes[18] = CreateTargetMode( 1680, 1050, 60 );
  TargetModes[19] = CreateTargetMode( 1600,  900, 60 );
  TargetModes[20] = CreateTargetMode( 1440,  900, 60 );
  TargetModes[21] = CreateTargetMode( 1400, 1050, 60 );
  TargetModes[22] = CreateTargetMode( 1366,  768, 60 );
  TargetModes[23] = CreateTargetMode( 1360,  768, 60 );
  TargetModes[24] = CreateTargetMode( 1280, 1024, 60 );
  TargetModes[25] = CreateTargetMode( 1280,  960, 60 );
  TargetModes[26] = CreateTargetMode( 1280,  800, 60 );
  TargetModes[27] = CreateTargetMode( 1280,  768, 60 );
  TargetModes[28] = CreateTargetMode( 1280,  720, 60 );
  TargetModes[29] = CreateTargetMode( 1280,  600, 60 );
  TargetModes[30] = CreateTargetMode( 1152,  864, 60 );
  TargetModes[31] = CreateTargetMode( 1024,  768, 60 );
  TargetModes[32] = CreateTargetMode(  800,  600, 60 );
  TargetModes[33] = CreateTargetMode(  640,  480, 60 );
...
_Use_decl_annotations_
static ::NTSTATUS MonitorAssignSwapChain( ::IDDCX_MONITOR MonitorObject, const ::IDARG_IN_SETSWAPCHAIN* pInArgs ) {
  DCWrapper* pContext = WdfObjectGet_DCWrapper(MonitorObject);
  pContext->pContext->AssignSwapChain(pInArgs->hSwapChain, pInArgs->RenderAdapterLuid, pInArgs->hNextSurfaceAvailable);
  return STATUS_SUCCESS;
}

_Use_decl_annotations_
static ::NTSTATUS MonitorUnassignSwapChain( ::IDDCX_MONITOR MonitorObject ) {
  DCWrapper* pContext = WdfObjectGet_DCWrapper(MonitorObject);
